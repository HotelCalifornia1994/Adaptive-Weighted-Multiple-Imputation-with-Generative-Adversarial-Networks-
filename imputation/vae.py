...
class TFVariationalAutoencoder(object):
    ...
    def _create_loss_optimizer(self):
        ...
        # 2.) The latent loss, which is defined as the Callback Libeler divergence
...
if __name__ == '__main__':
    # 加载数据
    ori_data_x, missing_data, data_m = data_loader('yiyue', 0.2, 1)
    # 标准化
    norm_data, norm_parameters = normalization(missing_data)
    # VAE network size:
    Decoder_hidden1 = 32
    Decoder_hidden2 = 32
    Encoder_hidden1 = 32
    ...
    network_architecture = \
        dict(n_hidden_recog_1=Encoder_hidden1,  # 1st layer encoder neurons
             ...
             n_input=norm_data.shape[1],  # data input size
    ...
    # impute missing values:
    X_impute = vae.impute(X_corrupt=norm_data, max_iter=ImputeIter)

    # Renormalization
    X_impute = renormalization(X_impute, norm_parameters)

    # Rounding
    imputed_data = rounding(X_impute, missing_data)
    # np.savetxt('vae.csv', imputed_data, delimiter=',')
    print(imputed_data)
