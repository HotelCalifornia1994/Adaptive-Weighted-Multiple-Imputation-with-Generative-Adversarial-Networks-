...
def gain(data_x, gain_parameters):
    ...
    D_W2 = tf.Variable(xavier_init([128, dim]))
    ...
    G_W2 = tf.Variable(xavier_init([128, dim]))
    ...
    for data_name in ['yiyue']:
        for missing_rate in [0.6]:
            for random_seed in [1, 2, 3]:
                ...
                #                f'/gaingai_{random_seed}.csv', index=False)
                # 存储数据
                # np.savetxt(f'D:/Desktop/我的插补/data/imputation_data/{missing_rate}missing_rate/{data_name}'
                #            f'/gaingai_{random_seed}.csv', imputed_data, delimiter=',')
