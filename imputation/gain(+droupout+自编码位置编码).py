...
def gain(data_x, gain_parameters):
    ...
    def generator(x, m, p):
        ...
        G_h1 = tf.nn.dropout(G_h1, 0.8)
        G_h1 = tf.add(G_h1, p)

        G_h2 = tf.matmul(G_h1, G_W2) + G_b2
        G_h2 = tf.nn.relu(G_h2)
        G_h2 = tf.nn.dropout(G_h2, 0.8)

        G_h3 = tf.matmul(G_h2, G_W3) + G_b3
        G_h3 = tf.nn.dropout(G_h3, 0.8)
...
if __name__ == '__main__':
    ...
    #     for missing_rate in [0.8]:
            for random_seed in [1, 2, 3]:
                ...
                # 存储评价指标
                metrics.to_csv(f'../data/Ablation/metrics/{missing_rate}missing_rate/{data_name}'
                               f'/gain(+droupout+自编码位置编码)_{random_seed}.csv', index=False)
                # 存储数据
                np.savetxt(f'../data/Ablation/imputation_data/{missing_rate}missing_rate/{data_name}'
                           f'/gain(+droupout+自编码位置编码)_{random_seed}.csv', imputed_data, delimiter=',')
